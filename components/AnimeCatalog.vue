<template>
  <v-card class="ota-anime-containers mt-2 mb-13" variant="text">
    <v-row no-gutters class="mt-2 flex-nowrap">
      <v-form @submit.prevent="submitForm" class="w-100">
        <v-text-field label="Что будем искать в мире аниме?" placeholder="например. Атака титанов" hide-details
          density="comfortable" autocomplete="off" clearable v-model="searchQuery"></v-text-field>
      </v-form>
    ...
    <v-row no-gutters class="mt-2 d-flex ga-2 flex-row flex-nowrap catalog-container">
        ...
        <v-list>
          <div v-if="animeCatalogListSkeleton">
            <v-card variant="text" class="pa-0" v-for="n in 10" :key="n">
              <v-skeleton-loader class="mb-2" type="list-item-three-line"></v-skeleton-loader>
            </v-card>
          </div>
          <div v-else ref="animeCatalog">
            <v-card v-for="anime in animeList" :key="anime.id" @click="openAnimeNotLoading(anime.id)" link rounded="0"
              ...
              <v-row no-gutters class="flex-nowrap">
                <div class="position-relative mr-3 catalog-content-containers-v-image">
                  <v-img :lazy-src="anime.poster.mainUrl" :src="anime.poster.mainUrl" rounded="lg" cover
                    class="pointer-events-none user-select-none overflow-hidden"></v-img>
          ...
          <v-card variant="tonal" v-if="!animeFound || !animeFound && !animeFoundInitialized"
      ...
      <v-card v-show="sidebarVisible" width="100%" class="sidebar" rounded="lg">
        <div class="content-wrapper d-flex flex-column">
          <v-form class="contents pt-2 position-relative" style="height: 100%; border-radius: 6px">
            <v-card variant="text" class="anime-filter-content-v-card pl-4 pr-4" width="100%">
              ...
              <v-select density="comfortable" variant="outlined" clearable class="mt-2" :item-props="itemProps"
                :items="statusItems" label="Выбирите Статус"></v-select>
            </v-card>
            <v-card variant="text" class="anime-filter-content-v-card pl-4 pr-4" width="100%">
              ...
              <v-select multiple chips density="comfortable" variant="outlined" clearable class="mt-2"
                :item-props="itemProps" :items="kindItems" label="Выбирите Тип"></v-select>
            </v-card>
            <v-card variant="text" class="anime-filter-content-v-card pl-4 pr-4" width="100%">
              ...
              <v-select multiple chips density="comfortable" variant="outlined" clearable class="mt-2"
                :item-props="itemProps" :items="genreItems" label="Выбирите Жанры"></v-select>
            </v-card>
            <v-card variant="text" class="anime-filter-content-v-card pl-4 pr-4" width="100%">
              ...
              <v-select density="comfortable" variant="outlined" clearable class="mt-2" :item-props="itemProps"
                :items="sortItems" label="Cортировать По"></v-select>
            </v-card>
            <v-card variant="text" class="anime-filter-content-v-card pl-4 pr-4" width="100%">
              ...
              <v-select density="comfortable" variant="outlined" clearable class="mt-2" :item-props="itemProps"
                :items="yearItems" label="Выбирите Сезон"></v-select>
            </v-card>
            <v-card variant="text" class="anime-filter-content-v-card pl-4 pr-4" width="100%">
              ...
              <v-select density="comfortable" variant="outlined" clearable class="mt-2" :item-props="itemProps"
                :items="ratingItems" label="Выбирите Оценку"></v-select>
            </v-card>
            <div class="submit-filter">
              <div class="submit-filter-inner-container pr-4 pl-4 d-flex ga-2 pb-2" style="border-radius: 6px;">
...
import { openAnimeNotLoading } from "~/typeScript/goTo";
