  ...
  <Swiper class="w-full bg-black main-slider" :modules="[SwiperAutoplay, SwiperEffectFade]" :slides-per-view="1"
    ...
    <SwiperSlide v-if="data" v-for="res in data" :key="res.id">
      <div class="w-full h-full relative">
        ...
        <div class="w-full lg:w-5/6 xl:w-3/5 flex flex-col items-start pr-2 pl-2 xl:pl-16 xl:pr-16 pb-5 md:pb-0 justify-end md:justify-center z-10 absolute left-0 h-full">
          ...
          <div class="flex opacity-65 mt-1 mb-1 text-sm md:text-base md:mt-3 md:mb-3">
            <p class="" v-for="(genre, index) in res.genres" :key="index">{{ genre }}<span v-if="index < res.genres.length - 1" class="mr-3 ml-3">•</span>
            </p>
          </div>
          <p class="text-sm md:text-base font-normal  opacity-80 md:font-semibold line-clamp-6">{{ res.description }}</p>
          <div class="mt-4 mb-1 flex-col gap-2 md:mt-3 md:mb-3 flex md:flex-row w-full md:w-auto">
            ...
            <NuxtLink class="button button-secondary"><svg class="w-5 h-5 fill-white" xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 384 512">
                <path
